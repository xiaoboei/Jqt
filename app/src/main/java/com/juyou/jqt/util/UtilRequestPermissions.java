...
public class UtilRequestPermissions {
     ...
     * 接收短信
     */
    public static final int REQUEST_CODE_RECEIVE_SMS = 0x9003;
    /**
     * 传感器
     */
     ...
     * H5获取照片存储
     */
    public static final int REQUEST_CODE_H5_READ_EXTERNAL_STORAGE= 0x9012;
    /**
     * H5获取照片相机
     */
    public static final int REQUEST_CODE_H5_CAMERA= 0x9013;
    /**
     * 请求权限
     * @param activity
     * @param permission
    ...
    public static void requestPermissionSms(Activity activity, int requestCode){
        if(Build.VERSION.SDK_INT >= 23){
            ...
            if(requestCode == 0){
                requestPermission(activity, new String[]{Manifest.permission.RECEIVE_SMS}, REQUEST_CODE_RECEIVE_SMS);
            } else {
                requestPermission(activity, new String[]{Manifest.permission.RECEIVE_SMS}, requestCode);
