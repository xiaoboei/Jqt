...
import com.juyou.jqt.util.UtilInteger;
...
final class CameraConfigurationManager {
	...
	private static Point findBestPreviewSizeValue(
		...
		for (String previewSize : COMMA_PATTERN.split(previewSizeValueString)) {
			...
			int newY;
			try {
				newX = UtilInteger.parseInt(previewSize.substring(0, dimPosition));
				newY = UtilInteger.parseInt(previewSize.substring(dimPosition + 1));
	...
	private void setZoom(Camera.Parameters parameters) {
		...
		if (takingPictureZoomMaxString != null) {
			try {
				int tenMaxZoom = UtilInteger.parseInt(takingPictureZoomMaxString);
				if (tenDesiredZoom > tenMaxZoom) {
					tenDesiredZoom = tenMaxZoom;
				}
			} catch (NumberFormatException nfe) {
